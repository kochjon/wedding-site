...
const CLOUD_NAME = 'dr9neirts';   // z.B. 'yeli-joni-wedding'
...
async function loadGallery() {
  ...
  try {
    const res = await fetch(
      `https://res.cloudinary.com/${CLOUD_NAME}/image/list/yeli-joni-wedding.json`
    );
    const data = await res.json();
    gallery.innerHTML = '';
    ...
    data.resources
      ...
      .forEach(img => {
        const url = `https://res.cloudinary.com/${CLOUD_NAME}/image/upload/${img.public_id}`;
